...
// let lemma Lemma (m: nat) (x: nat)//(usize 8) * (pad_len_5)
//   : Lemma (pow2 x < pow2 m)
//   = 

let nat_pow2_req (m:pos) (x: nat) = pow2 x < m
...
let poly1305_encode_last (pad_len_5: block_index_t) (b_6: sub_block_t) : field_element_t =
  let n_7 =
    uint128_from_le_bytes (array_from_slice (secret (pub_u8 0x0))
          ...
          (seq_len (b_6)))
  in
  let f_8: field_element_t = nat_from_secret_literal (0x03fffffffffffffffffffffffffffffffb) (n_7) in
...
let poly1305_update_last (pad_len_22: block_index_t) (b_23: sub_block_t) (st_24: poly_state_t)
    : poly_state_t =
  let st_25 = st_24 in
  let st_25 =
