    ...
    match &s {
        Statement::LetBinding((pat, pat_span), typ, ref expr, question_mark) => {
            ...
            let (new_expr, expr_typ, new_var_context) =
                typecheck_expression(sess, expr, &Some(return_typ), top_level_context, var_context)?;
        ...
        Statement::ReturnExp(e) => {
            ...
            let (new_e, e_t, var_context) =
                typecheck_expression(sess, &(e.clone(), s_span), &Some(return_typ), top_level_context, var_context)?;
            Ok((
        ...
        Statement::Conditional(cond, (b1, b1_span), b2, _) => {
            ...
            let (new_cond, cond_t, var_context) =
                typecheck_expression(sess, &cond, &Some(return_typ), top_level_context, var_context)?;
            unify_types_default_error_message(
                sess,
                &cond_t,
                &(
