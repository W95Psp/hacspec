...
fn typecheck_expression(
    ...
    match e {
	Expression::MonadicLet(..) => { // TODO: use GADT to eliminiate this `panic!`
		...
		let (_, qe_span) = **qe;
		let (qe, typ, var_context) = typecheck_expression(sess, qe, func_return_type, top_level_context, var_context)?;
		let carrier = typ.clone().1.0.try_into().unwrap();
		let typ = typecheck_question_mark(sess, true, typ, return_typ, qe_span, top_level_context)?;
		Ok((Expression::QuestionMark(
		    Box::new((qe, qe_span)),
		    Some(carrier)
		), typ, var_context))
	    } else {
        ...
        Expression::InlineConditional(cond, e_t, e_f) => {
            let (new_cond, t_cond, var_context) =
                typecheck_expression(sess, cond, func_return_type, top_level_context, &var_context)?;
            unify_types_default_error_message(
                sess,
                &t_cond,
                &(
        ...
        Expression::MethodCall(sel, _, (f, f_span), orig_args, _args_types) => {
            ...
            let (_, sel_typ, _) =
                typecheck_expression(sess, &sel, func_return_type, top_level_context, &var_context)?;
            let (f_sig, typ_var_ctx) = find_func(
                sess,
...
impl Into<BaseTyp> for CarrierTyp {
	...
	fn mk(name: &str, args: Vec<Spanned<BaseTyp>>) -> BaseTyp {
	    BaseTyp::Named((TopLevelIdent {
		string: name.to_string(),
		kind: TopLevelIdentKind::EnumConstructor
	    }, DUMMY_SP.into()), Some(args))
	}
	match self {
...
fn typecheck_expression_qm(
    ...
    let (e, ty, vctx) = typecheck_expression(sess, e, func_return_type, top_level_context, var_context)?;
...
fn typecheck_expression_no_qm(
    ...
    let (e, ty, carrier, vctx) = typecheck_expression_qm(sess, e, func_return_type, top_level_context, var_context)?;
    match carrier {
	Some(_) =>
	    Err(sess.span_rustspec_err(
		*e_span,
		format!(
		    "question mark cannot occur in this expression in Hacspec"
		).as_str(),
	    )),
	_ => Ok((e, ty, vctx))
    }
...
fn typecheck_statement(
    ...
    match &s {
        Statement::LetBinding((pat, pat_span), typ, ref expr, question_mark) => {
            ...
            #[cfg(feature = "dev")]
            log::trace!("   {:?}", backtrace::Backtrace::new());
            let (new_expr, expr_typ, carrier, new_var_context) =
                typecheck_expression_qm(sess, expr, &Some(return_typ), top_level_context, var_context)?;
        ...
        Statement::Reassignment((x, x_span), e, question_mark) => {
            ...
            let (new_e, e_typ, carrier, new_var_context) =
                typecheck_expression_qm(sess, &e, &Some(return_typ), top_level_context, var_context)?;
        ...
        Statement::ArrayUpdate((x, x_span), e1, e2, question_mark, _) => {
            ...
            let (new_e1, e1_t, carrier1, var_context) =
                typecheck_expression_qm(sess, &e1, &Some(return_typ), top_level_context, var_context)?;
            let (new_e2, e2_t, carrier2, var_context) =
                typecheck_expression_qm(sess, &e2, &Some(return_typ), top_level_context, &var_context)?;
	    ...
	    let question_mark = *question_mark || matches!(carrier1, Some(_)) || matches!(carrier2, Some(_));
        ...
        Statement::ReturnExp(e) => {
            ...
            let (new_e, e_t, var_context) =
                typecheck_expression_no_qm(sess, &(e.clone(), s_span), &Some(return_typ), top_level_context, var_context)?;
            Ok((
        ...
        Statement::Conditional(cond, (b1, b1_span), b2, _) => {
            ...
            let (new_cond, cond_t, var_context) =
                typecheck_expression_no_qm(sess, &cond, &Some(return_typ), top_level_context, var_context)?;
            unify_types_default_error_message(
                sess,
                &cond_t,
                &(
        ...
        Statement::ForLoop(x, e1, e2, (b, b_span)) => {
            ...
            let (new_e1, t_e1, var_context) =
                typecheck_expression_no_qm(sess, e1, &Some(return_typ), top_level_context, var_context)?;
            let (new_e2, t_e2, var_context) =
                typecheck_expression_no_qm(sess, e2, &Some(return_typ), top_level_context, &var_context)?;
            match (
                t_e1.0.clone(),
    ...
    let i = match &i {
        Item::NaturalIntegerDecl(typ_ident, secrecy, canvas_size, info) => {
            ...
            let (new_canvas_size, canvas_size_typ, _) =
                typecheck_expression_no_qm(sess, canvas_size, &None, top_level_context, &HashMap::new())?;
