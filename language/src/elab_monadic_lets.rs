...
pub fn eliminate_question_marks_in_expressions(
    e: &Expression,
) -> Expression {
    ...
    let mut elim_sub_expr0 = |arg: Expression| match eliminate_question_marks_in_expressions(&arg.clone()).clone() {
	Expression::MonadicLet(carrier, lbs, body, _) => {
	    bindings = Some(match bindings.clone() {
		None => 
		    (carrier, lbs.clone()),
		Some((carrier, prev_lbs)) =>
		    (carrier, prev_lbs.iter().chain(lbs.iter()).cloned().collect()),
	    });
	    body.0
	},
	arg => arg
    ...
    let mut elim_boxed_sub_expr = |arg: &Box<Spanned<Expression>>| Box::new(elim_sub_expr(*arg.clone()));
    fn find_monadic_let_carrier<T>(exprs: T) -> Option<CarrierTyp>
	where T: Iterator<Item=Expression> + Clone {
	...
	Expression::QuestionMark(e1, typ) => {
	    ...
	    // let mv = if let Expression::Named(id) = e1.clone().0 {
	    // 	(CollectIdents { idents = vec!() }).expr()
	    // 	id
	    // } else { fresh_var() };
	    let mv = fresh_var();
		...
		Expression::MonadicLet(
		    carrier,
		    bindings.into_iter().chain(std::iter::once(
			(mv.clone(), body.clone())
		    )).collect(),
		    mv_expr, pure
		)
		...
		Expression::MonadicLet(
		    typ.clone().expect("[QuestionMark] nodes in typed ADTs should always carry a type"),
	...
	Expression::FuncCall(prefix, name, args, arg_types) => {
		...
		args.into_iter().cloned().map(|(x, xb)| (elim_sub_expr(x), xb)).collect(),
		arg_types.clone()
	    )
	},
	Expression::MethodCall(self_, self_typ, name, args, arg_types) => {
		...
		self_typ.clone(), name.clone(),
		args.into_iter().cloned().map(|(x, xb)| (elim_sub_expr(x), xb)).collect(),
		arg_types.clone(),
	    )
	},
	Expression::EnumInject(t, n, e1) =>
	...
	Expression::InlineConditional(scrutinee, then_, else_) => {
	    ...
	    if let Some(carrier) = find_monadic_let_carrier(
		[then_.clone().0, else_.clone().0].iter().cloned()
	    ) {
	...
	Expression::MatchWith(scrutinee, branches) => {
	    let branches = branches.into_iter().cloned().map(
		|(t, cn, p, arm)| (t, cn, p, eliminate_question_marks_in_spanned_expressions(&arm))
	    );
	    let scrutinee = elim_boxed_sub_expr(scrutinee);
	    if let Some(carrier) = find_monadic_let_carrier(
		branches.clone().map(|(.., (x, _))| x)
	    ) {
		wrap_pure_node = false;
		let def = Box::new((
		    Expression::MatchWith(
			scrutinee,
			branches.map(|(t, cn, p, arm)| (t, cn, p, pure_if_non_monadic(carrier.clone(), arm))).collect()
		    ),
		    DUMMY_SP.into()
		));
		let mv = fresh_var();
    ...
    match bindings {
	Some((carrier, bindings)) =>
	    Expression::MonadicLet(
		carrier, bindings.clone(), Box::new((e, DUMMY_SP.into())),
		wrap_pure_node
	    ),
	_ => e	
    }
...
//     idents: Vec<Ident>
// }
// impl Mapper for CollectIdents {
//     fn ident(&mut self, e: &Ident) -> Ident {
// 	self.idents.extend(vec!(e.clone()));
// 	e.clone()
//     }
// }

// pub struct MonadicLetElab {
//     pub ret: Option<EarlyReturnType>
// }
// impl Mapper for MonadicLetElab {
//     fn expr(&mut self, e: &Expression) -> Expression {
// 	let e = eliminate_question_marks_in_expressions(e);
// 	if let Expression::MonadicLet(c,..) = e.clone() {
// 	    self.ret = Some(carrier_kind(c));
// 	}
// 	e
//     }
//     fn stmt(&mut self, s: &Statement) -> Statement {
// 	let s = self.stmt_default(s);
// 	// if let Some(c) = match s.clone() {
// 	//     Statement::LetBinding(_, _, (Expression::MonadicLet(c,..),_), _)
// 	//     Statement::Reassignment(_, (Expression::MonadicLet(c,..),_), _),
// 	//     Statement::Conditional((Expression::MonadicLet(c,..),_), _),
// 	// } {
// 	//     self.ret = Some(carrier_kind(c));
// 	// }
// 	match s.clone() {
// 	    Statement::LetBinding(pat, typ, (Expression::MonadicLet(c, bindings, body, true), _), false) =>
// 		match (bindings.as_slice(), body.0) {
// 		    ([(x, def)], Expression::Named(y)) if x.clone() == y => 
// 			Statement::LetBinding(pat, typ, *def.clone(), false),
// 		    _ => s.clone()
// 		},
// 	    Statement::LetBinding(pat, typ, e@(Expression::MonadicLet(..), _), false) =>
// 		Statement::LetBinding(pat, typ, e, true),
// 	    Statement::LetBinding(pat, typ, e@(Expression::MonadicLet(..), _), false) =>
// 		Statement::LetBinding(pat, typ, e, true),
// 	    Statement::Reassignment(x, e@(Expression::MonadicLet(..), _), false) =>
// 		Statement::Reassignment(x, e, true),
// 	    _ => s.clone()
// 	}
//     }
//     fn block(&mut self, b: &Block) -> Block {
// 	let mut s = MonadicLetElab {ret: None};
// 	let b = s.block_default(b);
// 	let mutated = *b.mutated.unwrap().clone();
// 	Block { mutated: Some(Box::new(MutatedInfo { early_return_type: mutated.early_return_type.or(s.ret), ..mutated })), ..b }
// 	// match b.clone() {
// 	//      => b
// 	// }
//     }
